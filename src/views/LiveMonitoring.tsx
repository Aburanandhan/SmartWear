...
export default function LiveMonitoring({
  ...
  const handleStopWorkout = async () => {
    ...
    const readingsToCalc = workoutReadings.length > 0 ? workoutReadings : [reading]
    const hrs = readingsToCalc.map((r) => r.heartRate || 140)
    const temps = readingsToCalc.map((r) => r.temperature || 37.0)
    const spo2s = readingsToCalc.map((r) => r.spo2 || 98)

    const avgHr = Math.round(hrs.reduce((a, b) => a + b, 0) / hrs.length)
    const maxHr = Math.max(...hrs)
    const avgTemp = (temps.reduce((a, b) => a + b, 0) / temps.length).toFixed(1)
    const minSpo2 = Math.min(...spo2s)
    const steps = Math.max(250, Math.round(elapsedSeconds * 2.2))
    ...
    const summary: CompletedWorkoutSummary = {
      ...
      calories,
      hydrationConsumedMl,
      intensity: avgHr > 150 ? 'High Intensity' : avgHr > 125 ? 'Moderate' : 'Light Recovery',
    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-teal-100 text-teal-800">
              {sensorSource.isSimulated ? 'Simulator Stream' : 'Physical BLE Stream'}
            </span>
          </div>
        ...
        <div className="space-y-6 fade-in">
          ...
          <div className="card p-5 bg-teal-50/50 border-teal-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="pulse-dot w-2.5 h-2.5 rounded-full bg-emerald-500" />
              <span className="font-bold text-sm text-teal-900" style={{ fontFamily: 'Sora, sans-serif' }}>
                LIVE TELEMETRY STREAM
              </span>
              <span className="text-xs text-teal-700 font-medium">· Viewing another person's live telemetry</span>
            </div>

            {/* Motion state switcher for simulator testing */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="card p-4 border-l-4 border-red-500">
              ...
              <div className="flex items-baseline gap-1">
                <span className="font-mono-data text-3xl font-bold text-red-500">{reading.heartRate}</span>
                <span className="text-xs text-slate-500 font-semibold">BPM</span>
              </div>
              <p className="text-xs text-slate-400 mt-2 truncate">State: {reading.motion}</p>
            </div>
            ...
            <div className="card p-4 border-l-4 border-teal-500">
              ...
              <div className="flex items-baseline gap-1">
                <span className="font-mono-data text-3xl font-bold text-teal-600">{reading.temperature}</span>
                <span className="text-xs text-slate-500 font-semibold">°C</span>
              </div>
              <p className="text-xs text-slate-400 mt-2 truncate">Continuous Probe</p>
            </div>
            ...
            <div className="card p-4 border-l-4 border-blue-500">
              ...
              <div className="flex items-baseline gap-1">
                <span className="font-mono-data text-3xl font-bold text-blue-600">{reading.spo2}</span>
                <span className="text-xs text-slate-500 font-semibold">%</span>
              </div>
              <p className="text-xs text-slate-400 mt-2 truncate">Pulse Oximeter</p>
            </div>
            ...
            <div className="card p-4 border-l-4 border-amber-500">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <span className="text-xl">
                  {MOTION_OPTIONS.find((m) => m.id === reading.motion)?.icon || '🏃'}
                </span>
                <span className="font-bold text-sm text-slate-900 truncate" style={{ fontFamily: 'Sora, sans-serif' }}>
                  {reading.motion}
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-2 truncate">{reading.steps} Total Steps</p>
            </div>
            ...
            <div className="card p-4 border-l-4 border-emerald-500">
              ...
              <p className="text-xs text-slate-400 mt-2 truncate">BLE 5.0 · 94% Batt</p>
            </div>
          </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400">Continuous telemetry feed (Heart Rate & Skin Temperature)</p>
              </div>
              <span className="text-xs text-teal-700 bg-teal-50 px-3 py-1 rounded-full font-mono-data font-semibold">
                Updated every 2.5s
              </span>
            </div>
      ...
      {activeTab === 'self' && (
        <div className="space-y-6 fade-in">
            ...
            {isWorkoutActive && (
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mt-6 pt-6 border-t border-slate-100 fade-in">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <p className="font-mono-data text-xl font-bold text-red-500">{reading.heartRate} <span className="text-xs">BPM</span></p>
                </div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <p className="text-xs text-slate-400 font-semibold uppercase">Body/Skin Temp</p>
                  <p className="font-mono-data text-xl font-bold text-teal-600">{reading.temperature}°C</p>
                </div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <p className="text-xs text-slate-400 font-semibold uppercase">SpO₂</p>
                  <p className="font-mono-data text-xl font-bold text-blue-600">{reading.spo2}%</p>
                </div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <p className="text-xs text-slate-400 font-semibold uppercase">Activity State</p>
                  <p className="font-mono-data text-base font-bold text-amber-600 truncate">{reading.motion}</p>
                </div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
          ...
          {/* POST WORKOUT SUMMARY — Rendered inside the SAME Live Monitoring page after stopping workout */}
          {lastSummary && (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                  ...
                  <p className="font-mono-data text-2xl font-bold text-red-400 mt-1">
                    {lastSummary.avgHr} <span className="text-xs text-slate-300 font-normal">/ {lastSummary.maxHr} BPM</span>
                  </p>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                  <p className="text-xs text-teal-200 uppercase font-semibold">Avg Body/Skin Temp</p>
                  <p className="font-mono-data text-2xl font-bold text-teal-300 mt-1">{lastSummary.avgTemp}°C</p>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                  <p className="text-xs text-teal-200 uppercase font-semibold">Lowest SpO₂</p>
                  <p className="font-mono-data text-2xl font-bold text-blue-300 mt-1">{lastSummary.minSpo2}%</p>
                </div>
                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
          ...
          {/* Telemetry Charts as Workout History & Trends */}
          <div className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-bold text-base text-slate-900" style={{ fontFamily: 'Sora, sans-serif' }}>
                ...
                <p className="text-xs text-slate-400">Real-time sensor trace captured during activity</p>
              </div>
              <span className="text-xs font-mono-data text-slate-400">Live feed active</span>
            </div>
